...
import { NutritionManager } from './js/features/nutrition.js?v=20260719-22';
import { HistoryManager, getDayMuscleColors } from './js/features/history.js?v=20260719-22';
...
export class FitTrackApp {
    constructor() {
        ...
        this.backupStorageKey = 'fittrack_automatic_backup';
        this.activeSectionKey = 'familygym_active_section';
    ...
    init() {
        ...
        this.renderBilbo();
        this.saveAutomaticBackup(false);
        const savedSection = window.location.hash.slice(1) || localStorage.getItem(this.activeSectionKey);
        if (savedSection && document.getElementById(`${savedSection}Section`)) this.switchSection(savedSection);
        this.routines.init();
        this.nutrition.init();
        this.history.init();
    ...
    setupEventListeners() {
        ...
        // El selector del historial se registra aquí, antes de los módulos opcionales.
        // Así sigue funcionando incluso si hay datos antiguos en otra tarjeta.
        document.getElementById('historyViewToggle')?.addEventListener('click', event => {
            const button = event.target.closest('[data-history-view]');
            if (!button) return;
            this.history.setView(button.dataset.historyView);
        });
        document.querySelector('.history-filters')?.addEventListener('click', event => {
            const button = event.target.closest('.history-filter-btn[data-filter]');
            if (!button) return;
            this.history.setFilter(button.dataset.type, button.dataset.filter);
        });
        document.getElementById('historySearch')?.addEventListener('input', event => {
            this.history.setQuery(event.target.value);
        });
        document.querySelectorAll('[data-home-section]').forEach(button => button.addEventListener('click', () => {
            this.switchSection(button.dataset.homeSection);
        }));
        requestAnimationFrame(() => this.updateNavigationIndicator(document.querySelector('.nav-btn.active')));
        window.addEventListener('resize', () => this.updateNavigationIndicator(document.querySelector('.nav-btn.active')));
        ...
        document.getElementById('activeWorkoutBtn')?.addEventListener('click', () => this.openActiveWorkout());
        document.getElementById('closeActiveWorkoutBtn')?.addEventListener('click', () => document.getElementById('activeWorkoutModal')?.close());
        document.getElementById('autoBackup')?.addEventListener('change', event => { this.settings.autoBackup = event.target.checked; this.saveSettings(); });
        document.getElementById('downloadBackupBtn')?.addEventListener('click', () => this.downloadBackup());
        document.getElementById('restoreBackupBtn')?.addEventListener('click', () => this.importData());
        ...
        document.getElementById('bilboWeightMinus').addEventListener('click', () => {
            if (this.bilboWeightPick > 5) {
                this.bilboWeightPick = Math.round(this.bilboWeightPick - 1);
        ...
        document.getElementById('bilboOrmInput').addEventListener('input', (e) => {
            ...
            if (orm > 0) {
                const half = Math.round(orm * 0.5); // En este gimnasio solo se usan kilos enteros.
    ...
    selectDate(date, formattedDateStr) {
        this.selectedDate = new Date(`${formattedDateStr}T12:00:00`);
    ...
    loadExercisesForDate(date) {
        ...
        this.elements.exercisesList.innerHTML = exercises.map((exercise, exerciseIndex) => {
            const muscleIndex = this.getExerciseMuscleIndex(exercise);
            const muscle = this.muscleGroups[muscleIndex] || 'Grupo muscular';
    ...
    // Each exercise belongs to the muscle group selected in the favourites
    // picker. Older records are detected from their first saved series.
    getExerciseMuscleIndex(exercise) {
        const savedIndex = this.muscleGroups.indexOf(exercise.muscleGroup);
        if (savedIndex >= 0) return savedIndex;
        const populatedIndex = (exercise.series || []).findIndex(group => (group?.sets || []).length > 0);
    ...
    selectFavoriteExercise(exerciseName, muscleGroup) {
        // Close the picker first so the next screen is the exercise panel,
        // ready to enter the first set rather than showing another menu.
        this.closeFavoriteModal(false);
    ...
    addExercise(exerciseName, muscleGroup = null) {
        ...
        const duplicateIndex = exercises.findIndex(item => item.name.trim().toLowerCase() === exerciseName.trim().toLowerCase() && item.muscleGroup === muscleGroup);
        if (duplicateIndex >= 0) {
            alert('Ese ejercicio ya está añadido para este grupo muscular.');
            document.getElementById(`exercisePanel${duplicateIndex}`)?.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
            return;
        }
        ...
        exercises.push({
            name: exerciseName,
            muscleGroup: this.muscleGroups[muscleIndex],
            series: newSeries,
    ...
    updateSetFromInput(input) {
        ...
        if (value && (!Number.isFinite(Number(value)) || Number(value) < 0 || (input.classList.contains('reps-input') && !Number.isInteger(Number(value))))) {
            alert(input.classList.contains('reps-input') ? 'Las repeticiones deben ser un número entero igual o mayor que 0.' : 'El peso debe ser un número igual o mayor que 0.');
            input.value = '';
            return;
        }
    ...
    deleteExercise(exerciseIndex) {
        if (!confirm('¿Eliminar este ejercicio y todas sus series?')) return;
        const exercises = this.getExercisesForDate(this.selectedDate);
    ...
    renderHomeDay() {
        ...
        this.renderHomeHighlights(today);
    }

    renderHomeHighlights(today) {
        const dateKey = formatDate(today);
        const todayExercises = Array.isArray(this.exercisesData?.[dateKey]) ? this.exercisesData[dateKey] : [];
        const todayCount = todayExercises.length;
        const heroMessage = todayCount ? `Tienes ${todayCount} ejercicio${todayCount === 1 ? '' : 's'} preparado${todayCount === 1 ? '' : 's'} para hoy.` : 'Todo empieza con una repetición.';
        const setText = (id, text) => { const element = document.getElementById(id); if (element) element.textContent = text; };
        setText('homeHeroMessage', heroMessage);
    }
    ...
    switchSection(section) {
        ...
        const previousName = activeSection?.id?.replace(/Section$/, '');
        const previousIndex = navButtons.findIndex(button => button.dataset.section === previousName);
        const nextIndex = navButtons.findIndex(button => button.dataset.section === section);
        const directionClass = previousIndex >= 0 && nextIndex >= 0 && nextIndex < previousIndex
        ...
        document.querySelectorAll('.section').forEach(s => s.classList.remove('section-enter-from-left', 'section-enter-from-right'));
        sectionElement.classList.add('active', directionClass);
        navButton?.classList.add('active');
        localStorage.setItem(this.activeSectionKey, section);
        window.history.replaceState(null, '', `#${section}`);
        if (navButton) this.updateNavigationIndicator(navButton);
    ...
    openActiveWorkout() {
        ...
        document.getElementById('activeWorkoutDate').textContent = this.selectedDate.toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long' });
    ...
    renderActiveWorkout() {
        ...
        const exercises = this.getExercisesForDate(this.selectedDate);
        if (!exercises.length) { list.innerHTML = '<p class="text-muted">Añade ejercicios al día desde el calendario para empezar.</p>'; return; }
        list.innerHTML = exercises.map((exercise, exerciseIndex) => {
            const muscleIndex = this.getExerciseMuscleIndex(exercise); const sets = exercise.series[muscleIndex].sets || [];
            return `<section class="active-exercise"><h3>${this.escapeHtml(exercise.name)} <small>${this.escapeHtml(this.muscleGroups[muscleIndex])}</small></h3><div class="active-sets">${sets.map((set, setIndex) => `<div class="active-set"><span>${setIndex + 1}</span><input type="number" data-active-exercise="${exerciseIndex}" data-active-muscle="${muscleIndex}" data-active-set="${setIndex}" data-active-field="weight" value="${set.weight || ''}" placeholder="kg" step="0.5"><input type="number" data-active-exercise="${exerciseIndex}" data-active-muscle="${muscleIndex}" data-active-set="${setIndex}" data-active-field="reps" value="${set.reps || ''}" placeholder="reps"></div>`).join('')}</div><button type="button" class="active-add-set" data-active-add="${exerciseIndex}" data-active-muscle="${muscleIndex}">+ Añadir serie</button></section>`;
        }).join('');
        list.querySelectorAll('[data-active-field]').forEach(input => input.addEventListener('change', event => { const input = event.target; const exercise = exercises[input.dataset.activeExercise]; exercise.series[input.dataset.activeMuscle].sets[input.dataset.activeSet][input.dataset.activeField] = input.value; this.saveExercises(); }));
        list.querySelectorAll('[data-active-add]').forEach(button => button.addEventListener('click', () => { exercises[button.dataset.activeAdd].series[button.dataset.activeMuscle].sets.push({ weight: '', reps: '' }); this.saveExercises(); this.renderActiveWorkout(); }));
    }

    updateNavigationIndicator(button) {
    ...
    // ==================== ESTADÍSTICAS ====================
    loadStats() {
        // Se pinta primero para que la comparación mensual no dependa de gráficos u otros paneles.
        this.renderMonthCompare();
    ...
    renderMonthCompare() {
        ...
        const exerciseData = this.exercisesData && typeof this.exercisesData === 'object' ? this.exercisesData : {};
        const cardioData = this.cardioData && typeof this.cardioData === 'object' ? this.cardioData : {};
        const getVolume = exercises => (Array.isArray(exercises) ? exercises : []).reduce((total, exercise) => total + (Array.isArray(exercise?.series) ? exercise.series : []).reduce((seriesTotal, muscle) => seriesTotal + (Array.isArray(muscle?.sets) ? muscle.sets : []).reduce((setsTotal, set) => setsTotal + (Number(set?.weight) || 0) * (Number(set?.reps) || 0), 0), 0), 0);

        Object.entries(exerciseData).forEach(([dateStr, exercises]) => {
            if (dateStr.startsWith(thisMonthKey)) { thisWorkouts++; thisVolume += getVolume(exercises); }
            else if (dateStr.startsWith(prevMonthKey)) { prevWorkouts++; prevVolume += getVolume(exercises); }
        });

        Object.entries(cardioData).forEach(([dateStr, entries]) => {
    ...
    exportWeightCSV() {
        ...
        a.href = url;
        a.download = `familygym_peso_${formatDate(new Date())}.csv`;
        a.click();
    ...
    exportMeasurementsCSV() {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `familygym_medidas_${formatDate(new Date())}.csv`;
        a.click();
        URL.revokeObjectURL(url);
    }
    ...
    exportJSON() {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `familygym_backup_${formatDate(new Date())}.json`;
        a.click();
        URL.revokeObjectURL(url);
    }
    ...
    createBackupSnapshot() {
        const data = {};
        Object.keys(localStorage).filter(key => key.startsWith('fittrack_') && key !== this.backupStorageKey && key !== 'fittrack_last_backup_day').forEach(key => {
            try { data[key] = JSON.parse(localStorage.getItem(key)); } catch { data[key] = localStorage.getItem(key); }
        });
        return { version: '3.0', createdAt: new Date().toISOString(), data };
    }

    saveAutomaticBackup(download = true) {
        const snapshot = this.createBackupSnapshot();
        localStorage.setItem(this.backupStorageKey, JSON.stringify(snapshot));
        const status = document.getElementById('backupStatus');
        if (status) status.textContent = `Última copia: ${new Date(snapshot.createdAt).toLocaleString('es-ES')}`;
        const today = formatDate(new Date());
        if (download && this.settings?.autoBackup && localStorage.getItem('fittrack_last_backup_day') !== today) {
            this.downloadBackup(snapshot, true);
            localStorage.setItem('fittrack_last_backup_day', today);
        }
    }

    downloadBackup(snapshot = this.createBackupSnapshot(), automatic = false) {
        const blob = new Blob([JSON.stringify(snapshot, null, 2)], { type: 'application/json' });
        const url = URL.createObjectURL(blob); const link = document.createElement('a');
        link.href = url; link.download = `familygym_copia_${formatDate(new Date())}.json`; link.click(); URL.revokeObjectURL(url);
        const status = document.getElementById('backupStatus');
        if (status) status.textContent = automatic ? 'Copia automática descargada hoy.' : 'Copia descargada correctamente.';
    }
    ...
    exportCSV() {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `familygym_export_${formatDate(new Date())}.csv`;
        a.click();
        URL.revokeObjectURL(url);
    }
    ...
    normalizeBilboData(rawData) {
        const defaults = this.getBilboDefaults();
    ...
    loadBilboData() {
        try {
            ...
            if (data) return this.normalizeBilboData(JSON.parse(data));
        } catch (e) {
            console.error('Error cargando Bilbo:', e);
        }
        return this.getBilboDefaults();
    }
    ...
    startBilbo() {
        const weight = Math.round(Number(this.bilboWeightPick));
        if (!weight || weight <= 0) {
    ...
    resetBilbo() {
        ...
        this.bilboData = this.getBilboDefaults();
        this.saveBilboData();
        this.renderBilbo();
    }
